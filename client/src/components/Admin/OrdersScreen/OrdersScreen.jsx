...
const OrderByIdScreen = () => {
    const { data: orders, isLoading: loadingOrders, refetch, error: ordersError } = useFetchAllOrdersQuery();
    ...
    return (
        <>
            ...
            <div className={"userScreenContent"}>
                    ...
                    {orders?.data.length > 0  && orders?.data?.map(order => (
                        <tr key={order.id}>
                            ...
                            <td>
                                ...
                                <Button variant={"light"} className={"btn-sm"}  onClick={() => handleFetchOrderById(order._id)}>
                                    View Details
                                </Button>
                            </td>
                        </tr>
                ...
                {(orders?.data?.length === 0 || orders === undefined)  && (<h4 className={"text-center mt-3"}>No Orders Yet!</h4>)}
            </div>
        </>
