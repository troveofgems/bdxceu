...
import "./Team.css"
const Team = () => {
    const {data: team, isLoading: isLoadingTeamList, error: teamListError, refetch: refetchTeamList} = useGetTeamQuery();

    return isLoadingTeamList ? <Preloader/> : (
                        ...
                        team.data.map((teamMember, index) => (
                            <div key={`${teamMember._id}_${index}`} className="wow fadeInUp col-md-4 col-sm-6" data-wow-delay="1.9s">
                                <div className="chiropractic-team-thumb">
                                    <img
                                        src={getSiteTeamHeadshotArtifact(`${teamMember.firstName.toLowerCase()}.${teamMember.lastName.toLowerCase()}`, "headshot")}
                                        className={getSiteTeamHeadshotArtifact(`${teamMember.firstName.toLowerCase()}.${teamMember.lastName.toLowerCase()}`, "appendCSS")}
                                        alt="chiropractic-team"
                                        height={getSiteTeamHeadshotArtifact(`${teamMember.firstName.toLowerCase()}.${teamMember.lastName.toLowerCase()}`, "imgDimensions.height")}
                                        width={getSiteTeamHeadshotArtifact(`${teamMember.firstName.toLowerCase()}.${teamMember.lastName.toLowerCase()}`, "imgDimensions.width")}/>
