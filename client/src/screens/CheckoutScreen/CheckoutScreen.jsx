...
const CheckoutScreen = () => {
  ...
  const [createBDXCEUOrder, { isLoading: isLoadingProcessOrder }] =
    useCreateBDXCEUOrderMutation();
  const [{ isPending }, paypalDispatch] = usePayPalScriptReducer();

  const {
    data: paypal,
    isLoading: isLoadingFetchPaypalClientId,
    error: paypalError,
  } = useGetPaypalClientIdQuery();
  const {
    data: product,
  ...
  useEffect(() => {
    if (
      !paypalError &&
      !isLoadingFetchPaypalClientId &&
      paypal.data.client_id
    ) {
  ...
  function onApprove(data, actions) {
    return actions.order.capture().then(async function (orderId, details) {
      try {
        const res = await createBDXCEUOrder({
          orderId,
          details,
          productId,
          paymentMethod,
        ...
        return navigate(`/checkout-success/${res.data._id}`, {
          state: { ...res.data },
        });
      } catch (err) {
        console.error(err);
  ...
  return isLoadingFetchPaypalClientId || isLoadingProductById ? (
    <Preloader />
  ) : (
    <Container>
