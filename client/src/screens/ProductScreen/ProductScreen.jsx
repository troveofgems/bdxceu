...
const ProductScreen = () => {
    ...
    useEffect(() => {
      if(!!product && !isLoadingProductById) {
          let instructorImageArtifacts = getSiteTeamHeadshots(`${product.data.courseInstructor.firstName.toLowerCase()}.${product.data.courseInstructor.lastName.toLowerCase()}`);
    ...
    return isLoadingProductById ? <Preloader /> : (
        <section id="product" className="parallax-section productByIdPage">
            <div id="product-header">
                <h3 className="wow bounceIn" data-wow-delay="0.9s">Let's Get You Signed Up For The</h3>
                <h1 className="wow fadeInUp" data-wow-delay="1.6s">{product.data.courseTitle}</h1>
            </div>
            <Row className={"productContainer"}>
                ...
                <Col lg={4} md={4} sm={12}>
                    <section id="chiropractic-team" className={"team-style-override"}>
                        <div id={"course-instructor"}>
                            <h3 className={"instructorHeader mb-0 pb-0"}>Course Instructor</h3>
                            <div className="chiropractic-team-thumb course-instructor-thumb">
                                {
                                    courseInstructorProfile !== null && (
                                        <img src={courseInstructorProfile.headshot}
                                ...
                                <div className="chiropractic-team-overlay">
                                    <div className="chiropractic-team-des">
                                        <h2>{product.data.courseInstructor.firstName} {product.data.courseInstructor.lastName}</h2>
                                        <h3>{product.data.courseInstructor.certificationList.join(", ")}</h3>
                                    </div>
                                </div>
                            </div>
                            <p>
                                {product.data.courseInstructor.description}
                            </p>
                        </div>
                    </section>
                </Col>
                ...
                <Col lg={4} md={4} sm={12}>
                    <div className="d-flex justify-content-center align-self-center">
                        <div className="blog-content wow fadeInUp" data-wow-delay="1s">
                            <Link to={`/product/${product.data._id}/checkout`}
